...
export default function OpenDocument() {
  ...
  const handleSubmit = async (e) => {
    ...
    const result = await axios.post(
      `http://localhost:8081/needDocInfo/${id}/${selectedItem.id}/`,
      newDocumentInfo
    );
  ...
  const deleteInfo = async (id) => {
    await axios.delete(`http://localhost:8081/needDocInfo/${id}`);
    loadDocumentInfo();
  };
